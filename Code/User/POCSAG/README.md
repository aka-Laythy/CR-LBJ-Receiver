...
| 字符内 bit 序 | MSB-first | **LSB-first**（`nibble_reverse()` 处理） |
...
- **char 内 nibble 反转**: TB/T 3504 规定每个 BCD 字符的 4 bit 是 LSB-first 传输的。`nibble_reverse()` 在解码前反转每个 nibble。
...
1. **bit 级偏移**: 前导码检测触发时如非 byte 对齐，首字节是垃圾，但 576 bit 前导码足够滑动对齐。
2. **2-bit+ 错误**: 仅做 1-bit 纠错。信道质量差时可能出现不可纠正码字，应检查射频链路。